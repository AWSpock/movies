...
async function loadData() {
  ...
  var tableRecent = document.getElementById("recent-movies");

  try {
    loader.show(true);

    var response = await fetch("/api/my-recent-movie", {
      method: "GET",
    });
    ...
    data.forEach(function (i) {
      ...
      tableRecent.appendChild(clone);

      x++;
    });

    var response = await fetch("/api/movie", {
      method: "GET",
    });
    // console.log(response);
